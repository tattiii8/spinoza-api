    ...
    public static async Task InitializeAsync(IServiceProvider services)
    {
        ...
        for (int retry = 1; retry <= maxRetries; retry++)
        {
            try
            {
                ...
                Console.WriteLine("[DatabaseInitializer] Database initialization completed successfully.");
            ...
            catch (Exception ex)
            {
                if (retry == maxRetries)
                {
                    Console.WriteLine($"[DatabaseInitializer] Failed to connect after {maxRetries} attempts.");
                    throw; // 規定回数を超えたら例外を投げて終了
                }

                Console.WriteLine($"[DatabaseInitializer] Waiting for database to be ready ({retry}/{maxRetries})... Error: {ex.Message}");
